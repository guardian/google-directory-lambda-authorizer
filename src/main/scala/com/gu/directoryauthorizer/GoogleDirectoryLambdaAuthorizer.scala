...
import com.gu.directoryauthorizer.ServiceAccountDirectoryAPIClient.{GroupKey, MemberKey, S3Path}
...
class GoogleDirectoryLambdaAuthorizer(tokenReader: GoogleOAuthTokenReader,
                                      directoryClient: GoogleDirectoryAPIClient,
  ...
  private[directoryauthorizer] def authorize(input: TokenInput): EitherT[Future, Throwable, AuthResponse] =
    for {
      ...
      allowed <- directoryClient.memberOfAll(requiredMembership.toList, MemberKey(email)).attemptT
    } yield {
      if (allowed) AuthResponse.allowMethodInvoke(email, input.methodArn)
      else AuthResponse.denyAll(email)
    }
  ...
  override def handleRequest(input: InputStream, output: OutputStream, context: Context): Unit = {
    ...
    output.write(authResponse.json.getBytes)
    output.close()
  }
}
...
object GoogleDirectoryLambdaAuthorizer {
  ...
  (implicit ec: ExecutionContext): GoogleDirectoryLambdaAuthorizer =
    ServiceAccountDirectoryAPIClient
